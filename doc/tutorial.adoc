...
In this tutorial, we will us two pipelines (custom pipeline and COLMAP pipeline) to explain how to localize query images within a map.
We will first explain how to build the map using structure-from-motion leveraging known camera poses.
Then, we will show how to localize query images and how to evaluate the precision of the obtained localization against the ground truth.
...
Note, a kapture folder contains sensor data and, if available, reconstruction data. More details about the structure of a kapture folder and the data it contains can be found in the link:https://github.com/naver/kapture/blob/master/kapture_format.adoc[kapture repository].
...
WARNING: Windows users need to have the file extension `.py` associated to the python3.6 executable special rights
...
Before going through the <<kapture pipeline>>, __local features__ and __global features__ have to be extracted for each image.
...
Features for mapping and query images should be in the same folder (see the <<recommended dataset structure>> above).
...
# For RobotCar or RobotCar_v2 --prepend_cam needs to be added.
...
In contrast to <<kapture pipeline>>, this section walks through a simpler pipeline fully based on COLMAP using SIFT local features
and Vocabulary Tree matching.

This tutorial also needs to be started from scratch. To clean unwanted files (e.g. previous experiments) see <<preparing data>>.
...
In this script, the --config option will set the parameters passed to the COLMAP image_registrator.
The parameters are described in link:../kapture_localization/colmap/colmap_command.py[colmap_command.py].
...
NOTE: In order to facilitate the use of our benchmark and to reduce processing load, we will soon provide COLMAP maps alongside R2D2 local features and several global features for all three benchmark datasets.
We will furthermore provide commands for executing the benchmark with the provided data.
...
Once everything in ready, the following example shows how the benchmark should be called.
...
`./image_retrieval_benchmark/r2d2_500/AP-GeM-LM18_top5/` contains the pairs file as well as the LTVL-style results and kapture-style `eval` results.
