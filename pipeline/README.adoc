...
- if your operating system cannot infer the python binary from the shebang or the file extensions (default behviour), you can explicitely give the python binary path with the `-python` parameter or let python find it from sys.executable with `--auto-python-binary`.
