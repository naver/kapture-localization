...
for SCENE in scene01 scene02 scene03 scene04 scene05 scene06 scene07 scene08 scene09 scene10; do 
  ...
  mkdir -p ${WORKING_DIR}/${DATASET}/validation/sensors
  cp ${DATASETS_PATH}/${DATASET}/validation/sensors/*.txt ${WORKING_DIR}/${DATASET}/validation/sensors/
  ln -s ${DATASETS_PATH}/${DATASET}/validation/sensors/records_data ${WORKING_DIR}/${DATASET}/validation/sensors/records_data

  # 2) Merge mapping and validation kaptures (this will make it easier to extract the local and global features and it will be used for the localization step)
  kapture_merge.py -v debug -f \
    -i ${WORKING_DIR}/${DATASET}/mapping ${WORKING_DIR}/${DATASET}/validation \
    -o ${WORKING_DIR}/${DATASET}/map_plus_validation \
    --image_transfer link_relative

  # 3) Extract global features (we will use AP-GeM here)
  cd ${WORKING_DIR}/deep-image-retrieval
  ${PYTHONBIN} -m dirtorch.extract_kapture --kapture-root ${WORKING_DIR}/${DATASET}/map_plus_validation/ --checkpoint dirtorch/data/Resnet101-AP-GeM-LM18.pt --gpu 0
  # move to right location
  mkdir -p ${WORKING_DIR}/${DATASET}/global_features/Resnet101-AP-GeM-LM18/global_features
  mv ${WORKING_DIR}/${DATASET}/map_plus_validation/reconstruction/global_features/Resnet101-AP-GeM-LM18/* ${WORKING_DIR}/${DATASET}/global_features/Resnet101-AP-GeM-LM18/global_features/
  rm -rf ${WORKING_DIR}/${DATASET}/map_plus_validation/reconstruction/global_features/Resnet101-AP-GeM-LM18

  # 4) Extract local features (we will use R2D2 here)
  cd ${WORKING_DIR}/r2d2
  ${PYTHONBIN} extract_kapture.py --model models/r2d2_WASF_N8_big.pt --kapture-root ${WORKING_DIR}/${DATASET}/map_plus_validation/ --top-k 20000 --max-size 1024
  # move to right location
  mkdir -p ${WORKING_DIR}/${DATASET}/local_features/r2d2_WASF_N8_big/descriptors
  mv ${WORKING_DIR}/${DATASET}/map_plus_validation/reconstruction/descriptors/r2d2_WASF_N8_big/* ${WORKING_DIR}/${DATASET}/local_features/r2d2_WASF_N8_big/descriptors/
  mkdir -p ${WORKING_DIR}/${DATASET}/local_features/r2d2_WASF_N8_big/keypoints
  mv ${WORKING_DIR}/${DATASET}/map_plus_validation/reconstruction/keypoints/r2d2_WASF_N8_big/* ${WORKING_DIR}/${DATASET}/local_features/r2d2_WASF_N8_big/keypoints/
  ...
  kapture_pipeline_localize.py -v debug -f \
    -i ${WORKING_DIR}/${DATASET}/mapping \
    --query ${WORKING_DIR}/${DATASET}/validation \
    -kpt ${WORKING_DIR}/${DATASET}/local_features/${LOCAL}/keypoints \
    -desc ${WORKING_DIR}/${DATASET}/local_features/${LOCAL}/descriptors \
    -gfeat ${WORKING_DIR}/${DATASET}/global_features/${GLOBAL}/global_features \
    -matches ${WORKING_DIR}/${DATASET}/local_features/${LOCAL}/NN_no_gv/matches \
  ...
  cat ${WORKING_DIR}/${DATASET}/colmap-localize/${LOCAL}/${GLOBAL}/LTVL2020_style_result.txt >> ${WORKING_DIR}/RIO10/LTVL2020_style_result_all_scenes_${LOCAL}_${GLOBAL}.txt
done